<?php
// =============================== Klasik Sponsors widget ======================================
class Klasik_SponsorsWidget extends WP_Widget {
    /** constructor */

	function Klasik_SponsorsWidget() {
		$widget_ops = array('classname' => 'widget_klasik_sponsors', 'description' => __('KlasikThemes Sponsors','klasik') );
		$this->WP_Widget('klasik-sponsors-widget', __('KlasikThemes Sponsors','klasik'), $widget_ops);
	}
    ...
    function widget($args, $instance) {		
		...
		if(!is_numeric($cols) || $cols < 1 || $cols > 6){
			$cols = 4;
					...
					$output .='<div class="klasik-sponsors-widget '.$customclass.'">';
						$output .='<div class="row">';
                            ...
                            $tpl .= '</div>';
							$tpl .= '</div>';
							$tpl = apply_filters( 'klasik_sponsors_item_template', $tpl );
							...
							if ($wp_query->have_posts()) : 
								...
								$cf_thumb = get_the_post_thumbnail($post->ID, 'widget-sponsors', array('class' => 'alignleft'));
								...
								if($x%$cols==0){
									...
									$posttext = '';
									if($longdesc>0){
										$excerpt = klasik_string_limit_char(get_the_content(), $longdesc);
									}else{
